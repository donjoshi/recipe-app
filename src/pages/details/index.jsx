...
export default function Details() {
    ...
    const { id } = useParams();
    const { recipeDetailsData, setRecipeDetailsData, favouritesList, handleAddToFavourites } = useContext(GlobalContext);
        ...
        <div className="flex flex-col gap-3">
            ...
            <div>
                <button onClick={() => handleAddToFavourites(recipeDetailsData?.recipe)} className="p-3 px-8 rounded-lg text-sm uppercase font-medium tracking-wider mt-3 inline=block shadow-md bg-black text-white">
                    {
                        favouritesList && favouritesList.length>0 && favouritesList.findIndex((item) => item.id === recipeDetailsData?.recipe?.id) !== -1 ?
            ...
            <div>
                <span className="text-2xl font-semibold text-black">Ingrediets:</span>
